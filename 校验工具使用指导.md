...
​	校验结果文件名称是由：校验进程号、表名称、表Letter、分区号组成
...
​	例如：表名 "T_test_SOME_032", 转换后的letter (0 + 1 000 1111 ) 

​	该二进制串 010001111 转换为16进制为 10f
...
| 名称         | 描述                                                 |      |
| ------------ | ---------------------------------------------------- | ---- |
| schema       | 当前校验的数据库名称(mysql)或数据库(opengauss)schema |      |
| table        | 当前校验表名称                                       |      |
| topic        | 校验当前表宿端产生的临时topic                        |      |
| partitiions  | 校验当前表产生的临时topic所对应的topic分区           |      |
| checkMode    | 当前校验模式，全量校验，增量校验                     |      |
| result       | 校验结果                                             |      |
| message      | 校验结果详细描述                                     |      |
| createTime   | 校验结果生成时间                                     |      |
| keyInsertSet | 校验产生的差异记录主键集合，目标端缺失数据           |      |
| keyUpdateSet | 校验产生的差异记录主键集合，目标端差异数据           |      |
| keyDeleteSet | 校验产生的差异记录主键集合，目标端多出数据           |      |
| repairInsert | 校验差异修复语句，目标端缺失数据修复                 |      |
| repairUpdate | 校验差异修复语句，目标端变更数据修复                 |      |
| repairDelete | 校验差异修复语句，目标端多余数据修复                 |      |
| process      | 本次校验进程号                                       |      |
...
| rules.row              | 配置行级过滤规则<br />行级规则会自动继承table规则类；行级规则允许配置多组行过滤规则；<br />规则等效于select * from table order by primaryKey asc limit offset,count; |
